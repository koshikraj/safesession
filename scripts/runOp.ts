...
import { getSafeSessionModule } from '../test/utils/setup'
...
const runOp = async () => {
  ...
  const moduleAddress = MODULE_ADDRESS ?? (await getSafeSessionModule().then((module) => module.getAddress()))
  ...
  if (DEBUG) {
    ...
    console.log('Using SafeSessionModule:', moduleAddress)
