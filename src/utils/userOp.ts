...
import { PackedUserOperationStruct as PackedUserOperation } from '../../typechain-types/contracts/SafeSessionModule'
...
export const buildSessionKeyUserOpTransaction = (
  from: string,
  call: Call,
  nonce: BigNumberish,
  sessionKey: string,
  ...
  const abi = [
    'function executeWithSessionKey((address target, uint256 value, bytes data) calldata call, address sessionKey) external',
  ]

  const callData = new ethers.Interface(abi).encodeFunctionData('executeWithSessionKey', [call, sessionKey])
